...
const httpRequestHistogram = new promClient.Histogram({
    ...
    buckets: [0.025, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.45, 0.5, 0.55, 0.6, 0.75, 1, 2.5, 3, 4, 5, 10, 30, 60],
    registers: [register]
})
...
let requestCount = 0;
let requestAmountToError = getPercentDecreaseAvailability();

app.use(async (req, res, next) => {
    if (!req.path.includes("/metrics")) {
        requestCount++;

        if (process.env.DECREASE_AVAILABILITY == "true") {
            console.log("DECREASING AVAILABILITY actual: "
                + requestCount + " target: " + requestAmountToError);

            try {
                if (requestCount % requestAmountToError < 1) {
                    console.log("Throwing error after " + requestCount +
                        " requests (chance is " + process.env.PERCENT_DECREASE_AVAILABILITY + "%)");

                    requestCount = 0;
                    requestAmountToError = getPercentDecreaseAvailability();
        ...
        if (process.env.INCREASE_LATENCY == "true") {
            let min = 400, max = 600;
            // Gera uma latência entre 400ms e 600ms
            let latency = generateRandomIntegerBetween(max, min);
            console.log("INCREASING LATENCY IN " + latency + "ms");
...
function generateRandomIntegerBetween(max, min) {
    return Math.floor(Math.random() * (max - min + 1) + min);
}

function getPercentDecreaseAvailability() {
    if (process.env.PERCENT_DECREASE_AVAILABILITY
            .replace(/\s+/g, '') // Remove espaços
            .match(/^[5-9]$|^[1-8][0-9]$|^[9][0-5]$/)) {

        let percentDecreaseAvailability = parseInt(process.env.PERCENT_DECREASE_AVAILABILITY);

        return (100 / generateRandomIntegerBetween(
            percentDecreaseAvailability + 4,
            percentDecreaseAvailability - 4));
    } else {
        console.error("The value of PERCENT_DECREASE_AVAILABILITY must be a number between 5 and 95.");
        return 0;
    }
}
